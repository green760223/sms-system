defmodule SmsSystem.StoresManagement do
  @moduledoc """
  The StoresManagement context.
  """
  ...
  # Repo.get!(Store, id)|> Repo.preload(:products)
  ...
  def list_products(store) do

    IO.inspect(store, label: "Getting products for store with ID")
    id = store.id
    IO.inspect(id, label: "===Store ID===")
    query =
      from p in Product,
        where: p.store_id == ^id,
        preload: [:store]
