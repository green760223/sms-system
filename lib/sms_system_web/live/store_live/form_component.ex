defmodule SmsSystemWeb.StoreLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:store_id]} type="text" label="Store ID" />
        <:actions>
  ...
  def update(%{store: store} = assigns, socket) do
    changeset = StoresManagement.change_store(store)
  ...
  def handle_event("validate", %{"store" => store_params}, socket) do
    changeset =
      ...
      |> StoresManagement.change_store(store_params)
  ...
  defp save_store(socket, :edit, store_params) do
    case StoresManagement.update_store(socket.assigns.store, store_params) do
      {:ok, store} ->
  ...
  defp save_store(socket, :new, store_params) do
    case StoresManagement.create_store(store_params) do
      {:ok, store} ->
        notify_parent({:saved, store})
